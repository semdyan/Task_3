import allure
from locators.athorization_locators import AuthFormsLocators, ButtonLocators
...
class ForgotPasswordPage(BasePage):
    ...
    def go_to_forgot_password_page(self):
        ...
        self.wait_and_click_element(ButtonLocators.LOGIN_BUTTON)
        self.wait_and_click_element(ButtonLocators.FORGOT_PASSWORD_LINK)

    @allure.step('Ввод почты и переход на форму восстановления пароля')
    def set_email_and_submit(self, email):
        email_input = self.wait_and_get_element(AuthFormsLocators.FORGOT_PASSWORD_EMAIL_INPUT)
        email_input.send_keys(email)
        self.wait_and_click_element(ButtonLocators.RESTORE_PASSWORD_BUTTON)
    ...
    def click_show_password_icon(self):
        self.wait_and_click_element(ButtonLocators.SHOW_PASSWORD_ICON)

    @allure.step('Рамка поля для ввода пароля подсвечивается')
